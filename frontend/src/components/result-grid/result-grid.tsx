...
export function ResultGrid(props: ResultGridProps) {
  ...
  const isImportSupported = sourceObjectSupportsAction(
    item,
    objectRef?.Kind,
    SourceAction.ImportData,
  );
  const isExportSupported =
    props.actions?.rawQuery != null ||
    sourceObjectSupportsAction(item, objectRef?.Kind, SourceAction.ViewRows);

  const editing = props.editing;
  const isRowUpdateSupported = sourceObjectSupportsAction(
    item,
    objectRef?.Kind,
    SourceAction.UpdateData,
  );
  const isRowDeleteSupported = sourceObjectSupportsAction(
    item,
    objectRef?.Kind,
    SourceAction.DeleteData,
  );
  const canEditRows =
    !!editing && (editing.allowRowUpdate ?? true) && isRowUpdateSupported;
  const canDeleteRows =
    !!editing &&
    (editing.allowRowDelete ?? true) &&
    isRowDeleteSupported &&
    objectRef != null;
  const isMockDataSupported =
    sourceObjectSupportsAction(
      item,
      objectRef?.Kind,
      SourceAction.GenerateMockData,
    ) &&
    (props.actions?.isMockDataGenerationAllowed ?? true);
  ...
  const [preselectedFormat, setPreselectedFormat] = useState<
    "csv" | "excel" | "ndjson" | undefined
  >(undefined);
  ...
  const selectedRowsData = useMemo(() => {
    ...
    selectedRows,
    toNamedRow,
    props.actions?.rawQuery,
    data.rows,
    data.columns,
  ]);

  const openExport = useCallback(
    (format?: "csv" | "excel" | "ndjson", exportAll?: boolean) => {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("menu:trigger-export", handler);
    };
  }, [openExport]);

  useEffect(() => {
    const handler = () => {
      if (isImportSupported && props.actions?.allowImport) setShowImport(true);
    };
    window.addEventListener("menu:trigger-import", handler);
    return () => {
      window.removeEventListener("menu:trigger-import", handler);
    };
  ...
  const columnDefs = useMemo(
    ...
    [
      data.columns,
      data.columnTypes,
      data.columnIsPrimary,
      data.columnIsForeignKey,
      canEditRows,
      sorting,
      t,
    ],
  ...
  const onCellClicked = useCallback(
    (e: CellClickedEvent) => {
      ...
      clickTimer.current = window.setTimeout(() => {
        if (value != null) {
          void copyToClipboard(String(value)).then((ok) => {
  ...
  const onCopyCell = useCallback(() => {
    ...
    if (value != null) {
      void copyToClipboard(String(value)).then((ok) => {
        if (ok) toast.success(t("copiedCellToClipboard"));
      });
    }
  ...
  const onCopyRow = useCallback(() => {
    ...
    if (row != null) {
      void copyToClipboard(row.join("\t")).then((ok) => {
        if (ok) toast.success(t("rowCopiedToClipboard"));
      });
    }
  }, [menuTarget, data.rows, t]);
  ...
  const onEditCell = useCallback(() => {
    ...
    apiRef.current?.startEditingCell({
      rowIndex: menuTarget.rowIndex,
      colKey: `c${menuTarget.colIndex}`,
    });
  }, [menuTarget]);
  ...
  const doDeleteRows = useCallback(
    async (indexes: number[]) => {
      ...
      for (const index of indexes) {
        ...
        if (!row) continue;
        const values = data.columns.map((col, i) => ({
          Key: col,
          Value: row[i],
        }));
        try {
  ...
  const selectionEnabled =
    props.editing != null || props.actions?.rawQuery != null;

  // Foreign-key navigation handler
  const onForeignKey = useMemo(() => {
    if (menuTarget == null || !props.foreignKeys?.onEntitySearch)
      return undefined;
    const col = data.columns[menuTarget.colIndex];
    if (!col || !(props.foreignKeys.isValidForeignKey?.(col) ?? false))
      return undefined;
  ...
  useGridShortcuts({
    ...
    onEditFocused: canEditRows
      ? () => {
          const c = apiRef.current?.getFocusedCell();
          if (c)
            apiRef.current?.startEditingCell({
              rowIndex: c.rowIndex,
              colKey: c.column.getColId(),
            });
        }
      : undefined,
    onDeleteFocused: canDeleteRows
      ? () => {
          const c = apiRef.current?.getFocusedCell();
          if (c) setPendingDeleteIndexes([c.rowIndex ?? 0]);
        }
      : undefined,
    onNextPage: props.pagination?.show
      ? () => {
          const cur = props.pagination?.currentPage ?? 1;
          const tp = Math.ceil(
            (props.pagination?.totalCount ?? 0) /
              (props.pagination?.pageSize ?? 100),
          );
          if (cur < tp) props.pagination?.onPageChange?.(cur + 1);
        }
      : undefined,
    onPrevPage: props.pagination?.show
      ? () => {
          const cur = props.pagination?.currentPage ?? 1;
          if (cur > 1) props.pagination?.onPageChange?.(cur - 1);
        }
      : undefined,
  });
  ...
  return (
    <div className="flex flex-col w-full h-full" data-testid="result-grid">
      ...
      <GridContextMenu
        ...
        onMockData={
          !props.limitContextMenu && isMockDataSupported && objectRef
            ? () => {
                setShowMockDataSheet(true);
              }
            : undefined
      ...
      >
        <div style={{ height: "100%", width: "100%" }}>
          <AgGridReact
            ...
            getRowId={(p) =>
              String((p.data as Record<string, string>).__rowIndex)
            }
          />
        </div>
      </GridContextMenu>
        ...
        <div className="flex justify-end items-center gap-4 px-2 py-1 flex-shrink-0">
          {totalCount != null && totalCount > 0 && (
            <div className="text-sm" data-testid="total-count-bottom">
              <span className="font-semibold">{t("totalCount")}</span>{" "}
              {formatNumber(totalCount, language)}
            </div>
          )}
          {isImportSupported && props.actions?.allowImport && (
            <Button
      ...
      <AlertDialog
        open={pendingDeleteIndexes != null}
        onOpenChange={(open) => {
          if (!open) setPendingDeleteIndexes(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("deleteRowConfirmTitle", {
                count: pendingDeleteIndexes?.length ?? 1,
              })}
            </AlertDialogTitle>
            <AlertDialogDescription>
